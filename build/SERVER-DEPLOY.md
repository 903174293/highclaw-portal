...
| 登录没有 Google/GitHub | 解压目录需有 `.env.production`（`make release` 会优先把仓库根 `.env.production` 打进包，无则用 `.env.production.example` 内容生成同名文件）；填 `GOOGLE_AUTH_ENABLED` 与 `GOOGLE_CLIENT_*` 等，**或**后台 Auth 开启。改后 `pm2 restart highclaw-portal --update-env` |
